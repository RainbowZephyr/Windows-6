...
int main (){
  ...
  */
	...
	if(directory[i*32] != 0x0){
	  ...
	  if(directory[i*32 + 6] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 7] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 8] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 9] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 10] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 11] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 12] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 13] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 14] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 15] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 16] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 17] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 18] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 19] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 20] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 21] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 22] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 23] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 24] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 25] != 0x0){
	    used++;
	  }
	  if(directory[i*32 + 26] != 0x0){
	    used++;
	  }
     ...
     *
     *      programName[0] = line[7];
     *      programName[1] = line[8];
     *      programName[2] = line[9];
     *      programName[3] = line[10];
     *      programName[4] = line[11];
     *      programName[5] = line[12];
     *
     *
     *      while(c<26){
     *	char text[512];
     *	interrupt(0x21, 0, "Enter your text> \0 ", 0, 0);
  ...
  interrupt(0x21, 8, programName, buffer , 0);
  interrupt(0x21, 3,programName, buffer, 0);
  interrupt(0x21, 0, buffer, 0, 0);
  } else {
    interrupt(0x21, 0, "Bad Command \0", 0, 0);
